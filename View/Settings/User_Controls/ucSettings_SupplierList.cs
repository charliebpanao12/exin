...
{
    public partial class ucSettings_SupplierList : UserControl
    {
        int supplierID;
        int supplierCode;
        string supplierName;

        public ucSettings_SupplierList()
        {
            InitializeComponent();
        ...
        private void ucSettings_SupplierList_Load(object sender, EventArgs e)
        {
        ...
        public void loadSupplierList()
        {
            ...
            filterSupplier = txtSearch.Text.Trim();
            ...
            Conns.mySqlCmd = new MySqlCommand("SELECT * FROM tbl_suppliers WHERE Category_Code LIKE '" + filterBusinessCategory + "%' AND (Supplier_Code LIKE '%" + filterSupplier + "%' OR Supplier_Name LIKE '%" + filterSupplier + "%' OR Supplier_Address LIKE '%" + filterSupplier + "%') AND Status LIKE '" + filterRecordStatus + "%' GROUP BY Supplier_Code ORDER BY Supplier_Code ASC LIMIT 100", Conns.mySqlconn);     // Create a query command
            ...
            if (numRows > 0)
            {
                ...
                while (Conns.mySqlDataReader.Read())    // Loop the records
                {
                    dgvRecords.Rows.Add(Conns.mySqlDataReader["ID"].ToString(),
                        ...
                        Conns.mySqlDataReader["Supplier_Code"].ToString(),
                        Conns.mySqlDataReader["Supplier_Name"].ToString(),
                        Conns.mySqlDataReader["Supplier_TIN"].ToString(),
                        Conns.mySqlDataReader["Supplier_Address"].ToString(),
                        Conns.mySqlDataReader["Supplier_Notes"].ToString(),
                        Conns.mySqlDataReader["Supplier_Notes2"].ToString(),
        ...
        {
            // Access Validation
            if (classValidateCredentials.validateCredentials(Global.userID, "System Settings", "Supplier List", "Add") == false) // Validate if the user has access to a certain system feature
            {
            ...
            frmSettings_SupplierList_New frmSettings_SupplierList_New = new frmSettings_SupplierList_New();
            frmSettings_SupplierList_New.addViewEdit = "Add";
            frmSettings_SupplierList_New.ShowDialog();
        }
        ...
        private void dgvRecords_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                supplierID = Convert.ToInt32(dgvRecords.Rows[e.RowIndex].Cells[0].Value);    // Get the details of the record
                supplierCode = Convert.ToInt32(dgvRecords.Rows[e.RowIndex].Cells[4].Value);    // Get the details of the record
                supplierName = dgvRecords.Rows[e.RowIndex].Cells[5].Value.ToString();    // Get the details of the record
                ...
                if (e.ColumnIndex == 1) // View the details
                {
                    ...
                    frmSettings_SupplierList_New frmSettings_SupplierList_New = new frmSettings_SupplierList_New();
                    frmSettings_SupplierList_New.addViewEdit = "View";
                    frmSettings_SupplierList_New.supplierID = supplierID;
                    frmSettings_SupplierList_New.supplierCode = supplierCode;
                    frmSettings_SupplierList_New.supplierName = supplierName;
                    frmSettings_SupplierList_New.ShowDialog();
                }
                else if (e.ColumnIndex == 2) // Delete the records
                {
                    // Access Validation
                    if (classValidateCredentials.validateCredentials(Global.userID, "System Settings", "Supplier List", "Delete") == false) // Validate if the user has access to a certain system feature
                    {
                        new classMsgBox().showMsgError("Access Denied!");
                        Application.OpenForms["frmSettings"].Activate(); // Bring the parent form to the front
                        return;
                    ...
                    Boolean recordExist = classGlobalFunctions.validateRecordExistence("SELECT * FROM tbl_transaction WHERE Supplier_Code=" + supplierCode + " AND Status='Active' LIMIT 1;");
                    if (recordExist == true)
                    {
                    ...
                    if (Global.msgConfirmation == true)
                    {
                        ...
                        MySqlConnect Conns = new MySqlConnect();    // Connect to the database
                        Conns.mySqlCmd = new MySqlCommand("UPDATE tbl_suppliers SET Status='Deleted' WHERE Supplier_Code=" + supplierCode + ";", Conns.mySqlconn);     // Create a query command
                ...
                {
                    // Access Validation
                    if (classValidateCredentials.validateCredentials(Global.userID, "System Settings", "Supplier List", "Edit") == false) // Validate if the user has access to a certain system feature
                    {
                        new classMsgBox().showMsgError("Access Denied!");
                        Application.OpenForms["frmSettings"].Activate(); // Bring the parent form to the front
                        return;
                    ...
                    frmSettings_SupplierList_New frmSettings_SupplierList_New = new frmSettings_SupplierList_New();
                    frmSettings_SupplierList_New.addViewEdit = "Edit";
                    frmSettings_SupplierList_New.supplierID = supplierID;
                    frmSettings_SupplierList_New.supplierCode = supplierCode;
                    frmSettings_SupplierList_New.supplierName = supplierName;
                    frmSettings_SupplierList_New.ShowDialog();
        ...
        {
            frmSettings_SupplierList_BatchDuplicate frmSettings_SupplierList_BatchDuplicate = new frmSettings_SupplierList_BatchDuplicate();
            frmSettings_SupplierList_BatchDuplicate.ShowDialog();
        }
    }
}
