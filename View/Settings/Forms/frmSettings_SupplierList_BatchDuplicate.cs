...
{
    public partial class frmSettings_SupplierList_BatchDuplicate : Form
    {
        public frmSettings_SupplierList_BatchDuplicate()
        {
            InitializeComponent();
        ...
        private void frmSettings_SupplierList_BatchDuplicate_Load(object sender, EventArgs e)
        {
            //***** Main Code
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (txtBusinessCategoryCode_From.Text.Trim() == "")
            {
                ...
                Application.OpenForms["frmSettings_SupplierList_BatchDuplicate"].Activate(); // Bring the parent form to the front  
                return;
            }
            // Validations
            ...
            Boolean recordExist = classGlobalFunctions.validateRecordExistence("SELECT * FROM tbl_suppliers WHERE Category_Code='" + txtBusinessCategoryCode_From.Text.Trim() + "' LIMIT 1;");
            ...
            recordExist = classGlobalFunctions.validateRecordExistence("SELECT * FROM tbl_suppliers WHERE Category_Code='" + txtBusinessCategoryCode_To.Text.Trim() + "' LIMIT 1;");
            if (recordExist == true)
            {
            ...
            if (Global.msgConfirmation == true)
            {
                ...
                Conns.mySqlCmd = new MySqlCommand("SELECT * FROM tbl_suppliers WHERE Category_Code='" + txtBusinessCategoryCode_From.Text.Trim() + "'", Conns.mySqlconn);     // Create a query command
                ...
                if (numRows > 0)
                {
                    ...
                    {
                        // Insert new record
                        Conns2.mySqlCmd = new MySqlCommand("INSERT INTO tbl_suppliers(Category_Code, Supplier_Code, Supplier_Name, Supplier_TIN, Supplier_Address, Supplier_Notes, Supplier_Notes2, Status, Remarks) " +
                        "VALUES (@Category_Code, @Supplier_Code, @Supplier_Name, @Supplier_TIN, @Supplier_Address, @Supplier_Notes, @Supplier_Notes2, @Status, @Remarks);", Conns2.mySqlconn);     // Create a query command                    
                        Conns2.mySqlCmd.Parameters.AddWithValue("Category_Code", txtBusinessCategoryCode_To.Text.Trim());
                        Conns2.mySqlCmd.Parameters.AddWithValue("Supplier_Code", Conns.mySqlDataReader["Supplier_Code"].ToString());
                        Conns2.mySqlCmd.Parameters.AddWithValue("Supplier_Name", Conns.mySqlDataReader["Supplier_Name"].ToString());
                        Conns2.mySqlCmd.Parameters.AddWithValue("Supplier_TIN", Conns.mySqlDataReader["Supplier_TIN"].ToString());
                        Conns2.mySqlCmd.Parameters.AddWithValue("Supplier_Address", Conns.mySqlDataReader["Supplier_Address"].ToString());
                        Conns2.mySqlCmd.Parameters.AddWithValue("Supplier_Notes", Conns.mySqlDataReader["Supplier_Notes"].ToString());
                        Conns2.mySqlCmd.Parameters.AddWithValue("Supplier_Notes2", Conns.mySqlDataReader["Supplier_Notes2"].ToString());
