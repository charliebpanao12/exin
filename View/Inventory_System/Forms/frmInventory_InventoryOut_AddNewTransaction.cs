...
{
    public partial class frmInventory_InventoryOut_AddNewTransaction : Form
    {
        ...
        public frmInventory_InventoryOut_AddNewTransaction()
        {
            InitializeComponent();
        ...
        public void deleteItem()
        {
            ...
            {
                if (panelCartItems.Controls[i] is ucInventory_InventoryOut_CartItem)
                {
                    ucInventory_InventoryOut_CartItem ucInventory_InventoryOut_CartItem = panelCartItems.Controls[i] as ucInventory_InventoryOut_CartItem;
                    if (ucInventory_InventoryOut_CartItem.deleteMe == true)
                    {
        ...
        private void generateNewTransactionID()
        {
            ...
            newTransactionID = 0;
            SQLStatement = "SELECT transaction_id FROM tbl_inventory_out_transactions ORDER BY transaction_id DESC LIMIT 1";
        ...
        private void generateNewDRnumber()
        {
            MySqlConnect Conns = new MySqlConnect();
            newDRNumber = 101;
            SQLStatement = "SELECT dr_no FROM tbl_inventory_out_transactions ORDER BY dr_no DESC LIMIT 1";
            Conns.mySqlCmd = new MySqlCommand(SQLStatement, Conns.mySqlconn);
            Conns.mySqlDataReader = Conns.mySqlCmd.ExecuteReader();
            DataTable dt = new DataTable();
            dt.Load(Conns.mySqlDataReader);
        ...
        private void generateNewInvoiceNumber()
        {
            ...
            SQLStatement = "SELECT invoice_number FROM tbl_inventory_out_transactions ORDER BY invoice_number DESC LIMIT 1";
            Conns.mySqlCmd = new MySqlCommand(SQLStatement, Conns.mySqlconn);
            Conns.mySqlDataReader = Conns.mySqlCmd.ExecuteReader();
            DataTable dt = new DataTable();
            dt.Load(Conns.mySqlDataReader);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (panelCartItems.Controls.Count == 0)
            {
                ...
                Application.OpenForms["frmInventory_InventoryOut_AddNewTransaction"].Activate(); // Bring the parent form to the front
                return;
            }
            ...
            {
                SQLStatement = "DELETE FROM tbl_inventory_out_transactions WHERE transaction_id = " + TransactionID/* + " AND category_code = " + Global.businessCategory + " AND unit_code = " + Global.businessUnitCode*/;
                Conns.mySqlCmd = new MySqlCommand(SQLStatement, Conns.mySqlconn);
                Conns.mySqlDataReader = Conns.mySqlCmd.ExecuteReader();
                ...
                for (var j = panelCartItems.Controls.Count - 1; j >= 0; j--)
                {
                    ...
                    if (panelCartItems.Controls[j] is ucInventory_InventoryOut_CartItem)
                    {
                        ucInventory_InventoryOut_CartItem ucInventory_InventoryOut_CartItem = panelCartItems.Controls[j] as ucInventory_InventoryOut_CartItem;
                        int productID = ucInventory_InventoryOut_CartItem.productID;
                        string barcode = ucInventory_InventoryOut_CartItem.barcode;
                        string sku = ucInventory_InventoryOut_CartItem.sku;
                        string productName = ucInventory_InventoryOut_CartItem.productName;
                        string unitOfMeasure = ucInventory_InventoryOut_CartItem.unitOfMeasure;
                        double quantity = ucInventory_InventoryOut_CartItem.quantity;
                        double unitPrice = ucInventory_InventoryOut_CartItem.unitPrice;
                        double sellingPrice = ucInventory_InventoryOut_CartItem.sellingPrice;
                        //double discount = ucInventory_InventoryOut_CartItem.discount;
                        double totalAmount = ucInventory_InventoryOut_CartItem.totalAmount;

                        Conns.mySqlCmd = new MySqlCommand("insert into tbl_inventory_out_transactions" +
                                    " (transaction_id, " +
                                    "category_code," +
                                    "unit_code," +
                                    "Project_Code," +
        ...
        private void dgvTransaction_CellContentDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex > 0)
            {
                ...
                frmInventory_InventoryOut_AddNewTransaction_Quantity frmInventory_InventoryOut_AddNewTransaction_Quantity = new frmInventory_InventoryOut_AddNewTransaction_Quantity();
                frmInventory_InventoryOut_AddNewTransaction_Quantity.ShowDialog();
                ...
                for (var i = panelCartItems.Controls.Count - 1; i >= 0; i--)
                {
                    if (panelCartItems.Controls[i] is ucInventory_InventoryIn_CartItem)
                    {
                        ...
                        if (ucInventory_InventoryOut_CartItem.productID == Convert.ToInt32(dgvTransaction.Rows[e.RowIndex].Cells[1].Value.ToString()))
                        {
                            ucInventory_InventoryOut_CartItem.quantity += Convert.ToDouble(frmInventory_InventoryOut_AddNewTransaction_Quantity.numtxtQuantity.Value);
                            ucInventory_InventoryOut_CartItem.txtQuantity.Text = "" + (Convert.ToDouble(ucInventory_InventoryOut_CartItem.txtQuantity.Text) + Convert.ToDouble(frmInventory_InventoryOut_AddNewTransaction_Quantity.numtxtQuantity.Value));
                            isExisting = true;
                            i = -1;
                        }
                    }
                ...
                if (isExisting == false)
                {
                    ucInventory_InventoryOut_CartItem ucInventory_InventoryOut_CartItem = new ucInventory_InventoryOut_CartItem();
                    ucInventory_InventoryOut_CartItem.Dock = DockStyle.Top;
                    ucInventory_InventoryOut_CartItem.productID = Convert.ToInt32(dgvTransaction.Rows[e.RowIndex].Cells[1].Value.ToString());
                    ucInventory_InventoryOut_CartItem.productName = dgvTransaction.Rows[e.RowIndex].Cells[4].Value.ToString();
                    ucInventory_InventoryOut_CartItem.unitOfMeasure = dgvTransaction.Rows[e.RowIndex].Cells[5].Value.ToString();
                    ucInventory_InventoryOut_CartItem.unitPrice = Convert.ToDouble(dgvTransaction.Rows[e.RowIndex].Cells[6].Value.ToString());
                    ucInventory_InventoryOut_CartItem.sellingPrice = Convert.ToDouble(dgvTransaction.Rows[e.RowIndex].Cells[7].Value.ToString());
                    ucInventory_InventoryOut_CartItem.barcode = dgvTransaction.Rows[e.RowIndex].Cells[8].Value.ToString();
                    ucInventory_InventoryOut_CartItem.sku = dgvTransaction.Rows[e.RowIndex].Cells[9].Value.ToString();
                    //ucInventory_InventoryOut_CartItem.discount = Convert.ToDouble(frmInventory_InventoryOut_AddNewTransaction_Quantity.numtxtDiscount.Value);
                    ucInventory_InventoryOut_CartItem.quantity = Convert.ToDouble(frmInventory_InventoryOut_AddNewTransaction_Quantity.numtxtQuantity.Value);
                    double totalcost = ucInventory_InventoryOut_CartItem.sellingPrice * ucInventory_InventoryOut_CartItem.quantity;
                    //double totalamount = totalcost - ucInventory_InventoryOut_CartItem.discount;
                    ucInventory_InventoryOut_CartItem.totalAmount = totalcost;
                    panelCartItems.Controls.Add(ucInventory_InventoryOut_CartItem);
                }
            }            
        }
        ...
        private void frmInventory_InventoryIn_AddNewTransaction_Load(object sender, EventArgs e)
        {
            ...
            while (Conns.mySqlDataReader.Read())
            {
                ...
                cboShipTo.Items.Add(Conns.mySqlDataReader["Project_Name"].ToString());
                    ...
                    {
                        lblTitle.Text = "Inventory Out - New Transaction";
                    }
                    else if (AddorEditorEditProduct == "Edit")
                    {
                        //Title Label
                        lblTitle.Text = "Inventory Out - Update Transaction";
                    ...
                    Conns.mySqlCmd = new MySqlCommand("SELECT distinct T1.product_id, T1.inventory_type, T1.item_name, T1.unit_of_measure, T1.unit_cost, T1.selling_price, T1.bar_code, T1.sku, T2.transaction_id, T3.Project_Code, T2.quantity, T2.discount, T2.category_code, T2.bill_to, T2.Category_Code, T2.ship_to, T2.date_time, T2.dr_no, T2.invoice_number, T2.invoice_date, T2.consignment, T2.business_style, T2.department, T2.phone, T2.memo, T2.prepared_by, T2.checked_by, T2.approved_by, T2.released_by, T2.received_by FROM " +
                                                         ...
                                                         "LEFT JOIN (SELECT transaction_id, category_code, Project_Code, product_id, quantity, discount, total_amount, bill_to, ship_to, date_time, dr_no, invoice_number, invoice_date, consignment, business_style, department, phone, memo, prepared_by, checked_by, approved_by, released_by, received_by, status FROM tbl_inventory_out_transactions) AS T2 ON T1.product_id = T2.product_id " +
                    ...
                    ucInventory_InventoryOut_CartItem ucInventory_InventoryOut_CartItem = new ucInventory_InventoryOut_CartItem();
                    ucInventory_InventoryOut_CartItem.Dock = DockStyle.Top;
                    ucInventory_InventoryOut_CartItem.productID = Convert.ToInt32(Conns.mySqlDataReader["product_id"].ToString());
                    ucInventory_InventoryOut_CartItem.productName = Conns.mySqlDataReader["item_name"].ToString();
                    ucInventory_InventoryOut_CartItem.unitOfMeasure = Conns.mySqlDataReader["unit_of_measure"].ToString();
                    ucInventory_InventoryOut_CartItem.unitPrice = Convert.ToDouble(Conns.mySqlDataReader["unit_cost"].ToString());
                    ucInventory_InventoryOut_CartItem.sellingPrice = Convert.ToDouble(Conns.mySqlDataReader["selling_price"].ToString());
                    ucInventory_InventoryOut_CartItem.barcode = Conns.mySqlDataReader["bar_code"].ToString();
                    ucInventory_InventoryOut_CartItem.sku = Conns.mySqlDataReader["sku"].ToString();
                    ucInventory_InventoryOut_CartItem.quantity = Convert.ToDouble(Conns.mySqlDataReader["quantity"].ToString());
                    ucInventory_InventoryOut_CartItem.discount = Convert.ToDouble(Conns.mySqlDataReader["discount"].ToString());
                    double totalcost = ucInventory_InventoryOut_CartItem.unitPrice * ucInventory_InventoryOut_CartItem.quantity;
                    double totalamount = totalcost - ucInventory_InventoryOut_CartItem.discount;
                    ucInventory_InventoryOut_CartItem.totalAmount = totalcost;
                    panelCartItems.Controls.Add(ucInventory_InventoryOut_CartItem);
                }
        ...
        private void cboInventoryTypeSearch_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            if (InventoryTypeSearch != null)
            {
                ...
                MySqlConnect Conns = new MySqlConnect();
                int i = 0;
                //SQLStatement = "SELECT product_id, barcode, sku, category_code, item_name FROM tbl_inventory_out_transactions WHERE inventory_type = '" + InventoryTypeSearch + "' AND status = 'Active' GROUP BY transaction_id ORDER BY transaction_id DESC";
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            if (Search != null || Search != " ")
            {
                ...
                "LEFT JOIN (SELECT transaction_id, product_id, quantity, discount, total_amount, status FROM tbl_inventory_out_transactions) AS T2 ON T1.product_id = T2.product_id WHERE T1.product_id = '" + Search + "' OR T1.item_name LIKE'%' '" + Search + "' '%' AND T2.inventory_type = '" + InventoryTypeSearch + "' AND T1.status = 'Active' GROUP BY T1.transaction_id ORDER BY T1.transaction_id DESC";
        ...
        {
            Application.OpenForms["frmInventory_InventoryOut_AddNewTransaction"].Refresh();
            Application.OpenForms["frmInventory_InventoryOut_AddNewTransaction"].Activate(); // Bring the parent form to the front
        }
        ...
        private void dgvTransaction_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 10)
            {
                ...
                if (Global.msgConfirmation == true)
                {
                    ...
                    Conns.mySqlCmd = new MySqlCommand("update tbl_inventory_out_transactions set status = 'Deleted' WHERE product_id = " + productID /*+ " AND transaction_id = " + TransactionID*/, Conns.mySqlconn);
                    Conns.mySqlCmd.ExecuteNonQuery();
