...
{
    public partial class frmSettings_ChartOfAccounts_BatchDuplicate : Form
    {
        public frmSettings_ChartOfAccounts_BatchDuplicate()
        {
            InitializeComponent();
        ...
        private void frmSettings_ChartOfAccounts_BatchDuplicate_Load(object sender, EventArgs e)
        {
            //***** Main Code
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (txtBusinessCategoryCode_From.Text.Trim() == "")
            {
                ...
                Application.OpenForms["frmSettings_ChartOfAccounts_BatchDuplicate"].Activate(); // Bring the parent form to the front  
                return;
            }
            // Validations
            ...
            Boolean recordExist = classGlobalFunctions.validateRecordExistence("SELECT * FROM tbl_chartofaccounts WHERE Category_Code='" + txtBusinessCategoryCode_From.Text.Trim() + "' LIMIT 1;");
            ...
            recordExist = classGlobalFunctions.validateRecordExistence("SELECT * FROM tbl_chartofaccounts WHERE Category_Code='" + txtBusinessCategoryCode_To.Text.Trim() + "' LIMIT 1;");
            if (recordExist == true)
            {
            ...
            if (Global.msgConfirmation == true)
            {
                ...
                Conns.mySqlCmd = new MySqlCommand("SELECT * FROM tbl_chartofaccounts WHERE Category_Code='"+ txtBusinessCategoryCode_From.Text.Trim() +"'", Conns.mySqlconn);     // Create a query command
                ...
                if (numRows > 0)
                {
                    ...
                    {
                        // Insert new record
                        Conns2.mySqlCmd = new MySqlCommand("INSERT INTO tbl_chartofaccounts(Category_Code, GLSL, GL_Account, GL, SL_Account, ExpandedAccounts, ExpandedAccountDesc, Added_By, Added_Date, Aging_Type, Status, Remarks) " +
                        "VALUES (@Category_Code, @GLSL, @GL_Account, @GL, @SL_Account, @ExpandedAccounts, @ExpandedAccountDesc, @Added_By, @Added_Date, @Aging_Type, @Status, @Remarks);", Conns2.mySqlconn);     // Create a query command                    
                        Conns2.mySqlCmd.Parameters.AddWithValue("Category_Code", txtBusinessCategoryCode_To.Text.Trim());
                        Conns2.mySqlCmd.Parameters.AddWithValue("GLSL", Conns.mySqlDataReader["GLSL"].ToString());
                        Conns2.mySqlCmd.Parameters.AddWithValue("GL_Account", Conns.mySqlDataReader["GL_Account"].ToString());
                        Conns2.mySqlCmd.Parameters.AddWithValue("GL", Conns.mySqlDataReader["GL"].ToString());
                        Conns2.mySqlCmd.Parameters.AddWithValue("SL_Account", Conns.mySqlDataReader["SL_Account"].ToString());
                        Conns2.mySqlCmd.Parameters.AddWithValue("ExpandedAccounts", Conns.mySqlDataReader["ExpandedAccounts"].ToString());
                        Conns2.mySqlCmd.Parameters.AddWithValue("ExpandedAccountDesc", Conns.mySqlDataReader["ExpandedAccountDesc"].ToString());
                        ...
                        Conns2.mySqlCmd.Parameters.AddWithValue("Aging_Type", Conns.mySqlDataReader["Aging_Type"].ToString());
