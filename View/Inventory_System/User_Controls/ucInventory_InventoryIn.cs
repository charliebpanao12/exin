...
{
    public partial class ucInventory_InventoryIn : UserControl
    {
        ...
        public ucInventory_InventoryIn()
        {        
            InitializeComponent();
        ...
        {
            dgvTransactionIn.Rows.Clear();
            dgvTransactionIn_details.Rows.Clear();
        }
        private void LoadData()
        {
            ...
            if (DateFrom <= DateTo)
            {
                ...
                SQLStatement = "SELECT T1.transaction_id, T1.date_time, T1.total_quantity, T1.product_id, T2.category_code, T2.unit_code, T1.status FROM" +
                               "(SELECT transaction_id, product_id, date_time, quantity, SUM(quantity) AS total_quantity, status FROM tbl_inventory_in_transactions WHERE DATE(date_time) BETWEEN '" + queryDateFrom + "' AND '" + queryDateTo + "' AND status<>'Deleted' GROUP BY transaction_id) as T1  "+
            ...
            if (NumberOfResults > 0)
            {
                ...
                while (Conns.mySqlDataReader.Read())
                {
                    ...
                    if (dgvTransactionIn.Rows.Count>0)
                    {
                        if (DateofDelivery != Conns.mySqlDataReader.GetDateTime("date_time").ToShortDateString())
                        {
                            dgvTransactionIn.Rows.Add(i, "Total Quantity", "", Math.Round(TotalQuantity));
                    ...
                    {
                        DateofDelivery = Conns.mySqlDataReader.GetDateTime("date_time").ToShortDateString();
                        dgvTransactionIn.Rows.Add(0, "", DateofDelivery, "");
                    }
                    ...
                    dgvTransactionIn.Rows.Add(i, Conns.mySqlDataReader["transaction_id"].ToString(), TimeofDelivery, Conns.mySqlDataReader["total_quantity"].ToString());

                    i++;
                }

                dgvTransactionIn.Rows.Add(i, "Total Quantity", "", Math.Round(TotalQuantity)); // Show the total quantity at the end of the row
            }
            else
            {
        ...
        {
            frmInventory_InventoryIn_AddNewTransaction frmInventory_InventoryIn_AddNewTransaction = new frmInventory_InventoryIn_AddNewTransaction();
            frmInventory_InventoryIn_AddNewTransaction.AddorEditorEditProduct = "Add";
            frmInventory_InventoryIn_AddNewTransaction.ShowDialog();
        }
        ...
        private void btnDeleteRecord_Click(object sender, EventArgs e)
        {
            ...
            if (TransactionID > 0)
            {
                ...
                if (Global.msgConfirmation == true)
                {
                    ...
                    Conns.mySqlCmd = new MySqlCommand("update tbl_inventory_in_transactions set status = 'Deleted' WHERE transaction_id =" + TransactionID, Conns.mySqlconn);
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            if (Search != null || Search != " ")
            {
                MySqlConnect Conns = new MySqlConnect();
                int i = 0;
                //SQLStatement = "SELECT transaction_id, date_time, SUM(quantity) AS quantity FROM tbl_inventory_in_transactions WHERE transaction_id = '" + Search + "' OR product_id = '" + Search + "' OR item_name LIKE'%' '" + Search + "' '%' AND status = 'Active' GROUP BY transaction_id ORDER BY transaction_id DESC";
                SQLStatement = "SELECT transaction_id, date_time, quantity, SUM(quantity) AS total_quantity FROM tbl_inventory_in_transactions WHERE transaction_id = '" + Search + "' OR product_id = '" + Search + "' OR item_name LIKE'%' '" + Search + "' '%' AND category_code='" + Global.inventoryBusinessCategoryCode + "' AND unit_code=" + Global.inventoryBusinessUnitCode + " AND DATE(date_time) BETWEEN '" + dtpDateFrom.Value.ToString("yyyy-MM-dd") + "' AND '" + dtpDateTo.Value.ToString("yyyy-MM-dd") + "' AND status != 'Deleted' GROUP BY transaction_id ORDER BY transaction_id DESC";
                //SQLStatement = "SELECT T1.transaction_id, T1.date_time, T1.total_quantity, T1.product_id, T2.category_code, T2.unit_code, T1.status FROM" +
                //              "(SELECT transaction_id, product_id, date_time, quantity, SUM(quantity) AS total_quantity, status FROM tbl_inventory_in_transactions WHERE category_code = '" + Global.businessCategory + "' AND unit_code = " + Global.businessUnitCode + " AND DATE(date_time) BETWEEN '" + queryDateFrom + "' AND '" + queryDateTo + "' AND status<>'Deleted' GROUP BY transaction_id) as T1  " +
                ...
                if (NumberOfResults > 0)
                {
                            ...
                            while (Conns.mySqlDataReader.Read())
                            {
                                ...
                                TimeofDelivery = Conns.mySqlDataReader.GetDateTime("date_time").ToShortTimeString();
                                i++;
                                dgvTransactionIn.Rows.Add(i, "", DateofDelivery, "");
                                dgvTransactionIn.Rows.Add(i, Conns.mySqlDataReader["transaction_id"].ToString(), TimeofDelivery, Conns.mySqlDataReader["quantity"].ToString());
      ...
      private void dgvTransactionIn_SelectionChanged(object sender, EventArgs e)
        ...
        private void InventoryInForm()
        {
            ...
            Conns.mySqlCmd = new MySqlCommand("SELECT T1.transaction_id, T2.Project_Code, T2.Project_Name, T1.bill_to, T1.ship_to, T1.date_time, T1.dr_no, T1.invoice_number, T1.invoice_date, T1.consignment, T1.business_style, T1.department, T1.phone, T1.memo, T1.prepared_by, T1.checked_by, T1.approved_by, T1.released_by, T1.received_by FROM " +
                                    "(SELECT transaction_id, Project_Code, category_code, bill_to, ship_to, date_time, dr_no, invoice_number, invoice_date, consignment, business_style, department, phone, memo, prepared_by, checked_by, approved_by, released_by, received_by, status FROM tbl_inventory_in_transactions) AS T1 " +
        ...
        private void dgvTransactionIn_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (Information.IsNumeric(dgvTransactionIn.Rows[e.RowIndex].Cells[1].Value))
            {
                TransactionID = Convert.ToInt32(dgvTransactionIn.Rows[e.RowIndex].Cells[1].Value);
                if (TransactionID > 0)
                {
                    MySqlConnect Conns = new MySqlConnect();
                    SQLStatement = "SELECT product_id, barcode, sku, item_name, unit_of_measure, quantity, unit_price, selling_price, discount, total_amount FROM tbl_inventory_in_transactions WHERE transaction_id =" + TransactionID + " AND status = 'Active'";
                    dgvTransactionIn_details.Rows.Clear();
                    Conns.mySqlCmd = new MySqlCommand(SQLStatement, Conns.mySqlconn);
                    Conns.mySqlDataReader = Conns.mySqlCmd.ExecuteReader();
                    while (Conns.mySqlDataReader.Read())
                    {
                        i++;
                        dgvTransactionIn_details.Rows.Add(i,
        ...
        private void btnEditTransaction_Click(object sender, EventArgs e)
        {
            if (TransactionID > 0)
            {
                ...
                frmInventory_InventoryIn_AddNewTransaction frmInventory_InventoryIn_AddNewTransaction = new frmInventory_InventoryIn_AddNewTransaction();
                frmInventory_InventoryIn_AddNewTransaction.AddorEditorEditProduct = "Edit";
                frmInventory_InventoryIn_AddNewTransaction.TransactionID = TransactionID;
                frmInventory_InventoryIn_AddNewTransaction.ShowDialog();
            }
            else
            {
