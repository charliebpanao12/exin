...
{
    public partial class ucSettings_Projects : UserControl
    {
        int projectID;

        public ucSettings_Projects()
        {
            InitializeComponent();
        ...
        private void ucSettings_Projects_Load(object sender, EventArgs e)
        {
            ...
            classGlobalFunctions.loadComboBoxItems(cboBusinessUnit, "Unit_Name", "SELECT DISTINCT Unit_Name FROM tbl_units WHERE Status='Active' ORDER BY Unit_Name ASC", "All");
        ...
        public void loadProjectList()
        {
            ...
            filterBusinessUnit = classGlobalFunctions.getComboBoxDetails("Unit_Code", "SELECT Unit_Code FROM tbl_units WHERE Unit_Name='" + cboBusinessUnit.Text + "' AND Status='Active' LIMIT 1");
            filterProjects = txtSearch.Text.Trim();
            ...
            Conns.mySqlCmd = new MySqlCommand("SELECT * FROM tbl_projects WHERE Category_Code LIKE '" + filterBusinessCategory + "%' AND Unit_Code LIKE '" + filterBusinessUnit + "%' AND (Project_Name LIKE '%" + filterProjects + "%' OR Project_Notes LIKE '%" + filterProjects + "%') AND Status LIKE '" + filterRecordStatus + "%' ORDER BY Project_Code ASC", Conns.mySqlconn);     // Create a query command
            ...
            if (numRows > 0)
            {
                ...
                while (Conns.mySqlDataReader.Read())    // Loop the records
                {
                    dgvRecords.Rows.Add(Conns.mySqlDataReader["ID"].ToString(),
                        ...
                        Conns.mySqlDataReader["Project_Code"].ToString(),
                        Conns.mySqlDataReader["Project_Name"].ToString(),
                        Conns.mySqlDataReader["Project_Notes"].ToString(),
                        Conns.mySqlDataReader["Start_Date"].ToString(),
                        Conns.mySqlDataReader["End_Date"].ToString(),
                        Conns.mySqlDataReader["Duration_Date"].ToString(),
                        Conns.mySqlDataReader["Duration_Days"].ToString(),
        ...
        {
            // Access Validation
            if (classValidateCredentials.validateCredentials(Global.userID, "System Settings", "Projects", "Add") == false) // Validate if the user has access to a certain system feature
            {
            ...
            frmSettings_Projects_New frmSettings_Projects_New = new frmSettings_Projects_New();
            frmSettings_Projects_New.addViewEdit = "Add";
            frmSettings_Projects_New.ShowDialog();
        }
        ...
        private void dgvRecords_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                projectID = Convert.ToInt32(dgvRecords.Rows[e.RowIndex].Cells[0].Value);    // Get the ID of the record
                ...
                if (e.ColumnIndex == 1) // View the details
                {
                    ...
                    frmSettings_Projects_New frmSettings_Projects_New = new frmSettings_Projects_New();
                    frmSettings_Projects_New.addViewEdit = "View";
                    frmSettings_Projects_New.projectID = projectID;
                    frmSettings_Projects_New.ShowDialog();
                }
                else if (e.ColumnIndex == 2) // Delete the records
                {
                    // Access Validation
                    if (classValidateCredentials.validateCredentials(Global.userID, "System Settings", "Projects", "Delete") == false) // Validate if the user has access to a certain system feature
                    {
                        new classMsgBox().showMsgError("Access Denied!");
                        Application.OpenForms["frmSettings"].Activate(); // Bring the parent form to the front
                        return;
                    ...
                    if (Global.msgConfirmation == true)
                    {
                        ...
                        MySqlConnect Conns = new MySqlConnect();    // Connect to the database
                        Conns.mySqlCmd = new MySqlCommand("UPDATE tbl_projects SET Status='Deleted' WHERE ID=" + projectID + ";", Conns.mySqlconn);     // Create a query command
                ...
                {
                    // Access Validation
                    if (classValidateCredentials.validateCredentials(Global.userID, "System Settings", "Projects", "Edit") == false) // Validate if the user has access to a certain system feature
                    {
                        new classMsgBox().showMsgError("Access Denied!");
                        Application.OpenForms["frmSettings"].Activate(); // Bring the parent form to the front
                        return;
                    ...
                    frmSettings_Projects_New frmSettings_Projects_New = new frmSettings_Projects_New();
                    frmSettings_Projects_New.addViewEdit = "Edit";
                    frmSettings_Projects_New.projectID = projectID;
                    frmSettings_Projects_New.ShowDialog();
                }
            }
