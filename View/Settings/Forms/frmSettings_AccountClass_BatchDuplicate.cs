...
{
    public partial class frmSettings_AccountClass_BatchDuplicate : Form
    {
        public frmSettings_AccountClass_BatchDuplicate()
        {
            InitializeComponent();
        ...
        private void frmSettings_AccountClass_BatchDuplicate_Load(object sender, EventArgs e)
        {
            //***** Main Code
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (txtBusinessCategoryCode_From.Text.Trim() == "")
            {
                ...
                Application.OpenForms["frmSettings_AccountClass_BatchDuplicate"].Activate(); // Bring the parent form to the front  
                return;
            }
            // Validations
            ...
            Boolean recordExist = classGlobalFunctions.validateRecordExistence("SELECT * FROM tbl_accountclass WHERE Category_Code='" + txtBusinessCategoryCode_From.Text.Trim() + "' LIMIT 1;");
            ...
            recordExist = classGlobalFunctions.validateRecordExistence("SELECT * FROM tbl_accountclass WHERE Category_Code='" + txtBusinessCategoryCode_To.Text.Trim() + "' LIMIT 1;");
            if (recordExist == true)
            {
            ...
            if (Global.msgConfirmation == true)
            {
                ...
                Conns.mySqlCmd = new MySqlCommand("SELECT * FROM tbl_accountclass WHERE Category_Code='" + txtBusinessCategoryCode_From.Text.Trim() + "'", Conns.mySqlconn);     // Create a query command
                ...
                if (numRows > 0)
                {
                    ...
                    while (Conns.mySqlDataReader.Read())    // Get the records
                    {
                        ...
                        Conns2.mySqlCmd.Parameters.AddWithValue("AccountClass_Code", Conns.mySqlDataReader["AccountClass_Code"].ToString());
                        Conns2.mySqlCmd.Parameters.AddWithValue("AccountClass_Name", Conns.mySqlDataReader["AccountClass_Name"].ToString());
                        Conns2.mySqlCmd.Parameters.AddWithValue("AccountClass_Notes", Conns.mySqlDataReader["AccountClass_Notes"].ToString());
